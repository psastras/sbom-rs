...
fn test_cargo_sbom_spdx_3_0_1_output() {
  ...
  let has_creation_info = graph
    .iter()
    .any(|element| element["type"] == "CreationInfo");
  assert!(
    has_creation_info,
    "SPDX 3.0.1 output should contain a CreationInfo element"
  );

  // Verify we have software_Package elements
  let has_package = graph
    .iter()
    .any(|element| element["type"] == "software_Package");
  assert!(
    has_package,
    "SPDX 3.0.1 output should contain software_Package elements"
  );
}
